...
2.  **複素内積によるスコア計算 (Conjugate Product)**:
    ...
    > [!WARNING]
    > **過去の実験結果に関する注記 (2026/01/15追記)**
    > 本レポートにある実験結果（以下参照）は、実装ミスにより **最終層のSoftmaxにおいて複素数の「振幅 (Absolute Value)」のみを使用 (`softmax_real_with_abs`) していた** 状態のものです。
    > 本来は、複素数の実部 (`Re(z)`) を用いて確率化すべきでしたが、誤って振幅を用いていました。したがって、以下の結果は「位相情報を考慮したAttention」を用いつつも、最終分類段で位相情報を捨ててしまった設定での性能となります。修正後のコードでは `softmax_real_with_real` を使用しています。
